...
async def seed_demo(token: str, request: Request):
    ...
    if existing:
        return HTMLResponse("<h2>Already seeded. Demo account: demo@northstar-demo.com / NorthStarDemo2026!</h2>")
    ...
    db.execute("INSERT INTO users (email, full_name, password_hash, role, status) VALUES (?,?,?,?,?)",
        ("demo@northstar-demo.com", "Mary Johnson", hash_password("NorthStarDemo2026!"), "user", "active"))
    uid = db.query_one("SELECT id FROM users WHERE email = ?", ("demo@northstar-demo.com",))["id"]
    ...
    db.execute("INSERT INTO insurance_policies (beneficiary_profile_id,policy_type,insurance_company,plan_name,member_id,plan_number,effective_date,rx_bin) VALUES (?,?,?,?,?,?,?,?)",
        (pid,"Medicare Advantage","Humana","Gold Plus H5619",encrypt_text("H5619-4821"),"H5619-003","2026-01-01","610014"))
    db.execute("INSERT INTO insurance_policies (beneficiary_profile_id,policy_type,insurance_company,plan_name,member_id,effective_date) VALUES (?,?,?,?,?,?)",
        (pid,"Part D Prescription Drug Plan","SilverScript","SilverScript Choice",encrypt_text("SS-9234"),"2026-01-01"))
    ...
    return HTMLResponse("""
        ...
        ✅ Demo seeded successfully!<br><br>
        <span style="font-size:1rem;font-weight:normal;">
        Email: demo@northstar-demo.com<br>
        Password: NorthStarDemo2026!<br><br>
        <a href="/">Go to NorthStar</a>
        </span></h2>""")
